...
if fid<0
    errordlg('Specified file cannot be found','Error','modal')
    data=[];
    return
elseif ischar(fid)
    data=[];
...
switch data.eldiz
    case 1 %wenner
        ...
        for j=1:data.nd
            data.pat.esira(j,:)=[find(xelek==xc1(j)) find(xelek==xc2(j)) find(xelek==xp1(j)) find(xelek==xp2(j))];%Elektrotlarýn sýra numaralarý
        end
    ...
    case 6 %Pole Dipole
        ...
        switch data.mp
            case 1
                ...
                data.pat.kon=[xc1 xp1 xp2];%Ölçüm sýrasýnda elektrotlarýn
                %         konumlandýrýldýklarý noktanýn x koordinatlarý
            case 0
                xc1=data.xd;
                xp1=xc1+data.nlev.*data.ela;
                xp2=xp1+data.mn;
        ...
        for j=1:data.nd
            data.pat.esira(j,:)=[find(xelek==xc1(j)) find(xelek==xp1(j)) find(xelek==xp2(j))];%Elektrotlarýn sýra numaralarý
        end
        data.nel=length(xelek);
        data.xelek=xelek;
        data.pat.ind1=sub2ind([data.nel data.nel],data.pat.esira(:,1),data.pat.esira(:,2));
